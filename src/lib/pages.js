// this file is where you define various utility functions to fetch and process page data using the GraphQL queries you've defined in data/pages.js.
...
export async function getPageByUri(uri) {
  ...
  if (process.env.WORDPRESS_PLUGIN_SEO === 'true') {
    // Make sure this is a string comparison
    try {
...
export function mapPageData(page = {}) {
  console.log('Page before mapping:', page); // Log the page object before mapping
  const data = { ...page };
  ...
  // Map flexible content fields
  if (data.flex && data.flex.flex) {
    data.flex = data.flex.flex.map((flexItem) => {
      // Perform any necessary transformations on each flexible content item
      // For now, we just return the item as it is
      return flexItem;
    });
  }
  ...
  if (data.header) {
    data.header = {
      backgroundColour: data.header.backgroundColour,
      bigTitleTop: data.header.bigTitleTop,
      bigTitleBottom: data.header.bigTitleBottom,
      headerStyle: data.header.headerStyle,
      fieldGroupName: data.header.fieldGroupName,
      smallTitle: data.header.smallTitle,
    };
  }
...
export function getBreadcrumbsByUri(uri, pages) {
  ...
  do {
    ...
    // If the breadcrumb is the active page, we want to pass undefined for the uri to
